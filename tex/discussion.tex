The basic model, which implements a fully connected network, where any agent can interact with any other and both agents have the same winning chance, produces a wealth distribution which follows an exponential decay, given by Eq. (\ref{eq:generalExpo}).
...
\begin{figure}[H]
    ...
    \scalebox{.7}{\input{./img/realData_head.pgf}}
	 \caption{\emph{Wealth distribution in a logarithmic scale to show the exponential trend at small values of capital. From the fit we obtain an estimate for the exponent of $\la = -1.2 \times 10^{-5}$.}}
    \label{fig:real_head}
\end{figure}
\begin{figure}[H]
    \centering
