...
That is close to the real one, i.e. $\frac{9}{10}$: this error is due to the fact that we are computing the surface of the simplex with an exact formula, whereas the microstates are discrete.
Nonetheless, the error vanishes in the limit where the number of agents gets very large because also the number of microstates gets large.
