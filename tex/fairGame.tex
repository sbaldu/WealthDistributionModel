...
Now let's take $10^4$ players and give 5 coins each, then simulate a million of fair matches.
At the end of the simulation, let's plot the frequency of agents in respect to their capital and fit \cite{root} this plot with the function $y(x) = Ae^{Bx}$ as shown in Fig. (\ref{fig:expo}).
From Eq. \ref{eq:expo} we expect $A = -B = \frac{1}{5}$.
\begin{figure}[ht!]
    \centering
    ...
    \label{fig:expo}
\end{figure}
The fit gives the parameters $A = 0.29$ and $B = -0.22$, so they are compatible with the expected value $\frac{1}{5} = 0.20$.